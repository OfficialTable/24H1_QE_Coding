...
Tensor* Softmax::backward(Tensor* out){
  ...
  for(int n=0; n<N; ++n){
    ...
    grad_val[0] = p0*(1-p0)*out->getGrad(n,0)-p1*p0*out->getGrad(n,1)-p2*p0*out->getGrad(n,2);
    grad_val[1] = p1*(1-p1)*out->getGrad(n,1)-p1*p0*out->getGrad(n,0)-p1*p2*out->getGrad(n,2);
    grad_val[2] = p2*(1-p2)*out->getGrad(n,2)-p2*p0*out->getGrad(n,0)-p2*p1*out->getGrad(n,1);
...
void Loss::backward(Tensor *out){
  ...
  for (int n=0; n<n_; ++n){
    out->setGrad(n,0,(-dloss/(double)(n_))*(ctx_[n*6+0]/ctx_[n*6+3]));
    out->setGrad(n,1,(-dloss/(double)(n_))*(ctx_[n*6+1]/ctx_[n*6+4]));
    out->setGrad(n,2,(-dloss/(double)(n_))*(ctx_[n*6+2]/ctx_[n*6+5]));
  }
}
