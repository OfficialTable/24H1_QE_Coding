...
Tensor* Softmax::backward(Tensor* out){
  ...
  for(int n=0; n<N; ++n){
    ...
    double grad0 = out->getGrad(n, 0);
    double grad1 = out->getGrad(n, 1);
    double grad2 = out->getGrad(n, 2);
    
    grad_val[0] = grad0 * (p0 * (1 - p0)) - grad1 * (p0 * p1) - grad2 * (p0 * p2);
    grad_val[1] = grad1 * (p1 * (1 - p1)) - grad2 * (p1 * p2) - grad0 * (p1 * p0);
    grad_val[2] = grad2 * (p2 * (1 - p2)) - grad0 * (p2 * p0) - grad1 * (p2 * p1);
...
Tensor* Linear::backward(Tensor* out){	
  ...
  for(int n=0; n<N; ++n){
    for(int i=0; i<ic_; ++i){
      ...
      // TODO: calculate tmp
      for(int o=0; o<oc_; ++o){
      tmp += out->getGrad(n,o) * weight_->getData(i, o);
      }
  ...
  for(int i=0; i<ic_; ++i){
    for(int o=0; o<oc_; ++o){
      ...
      for(int n=0; n<N; ++n){
      tmp_grad += out->getGrad(n,o) * in_->getData(n, i);
      }
...
void Loss::backward(Tensor *out){
  ...
  for(int n=0; n<N; ++n){
    for(int w=0; w<W; ++w){
      grad = dloss * (- 1.0) * (ctx_[n*6 + w] /ctx_[n*6 + (w + 3)]) / N;
      out->setGrad(n, w, grad);
      }
    }
}
